...
function MessageBubble({ msg }) {
  ...
  const renderContent = (text) => {
    ...
    return lines.map((line, i) => {
      ...
      if (isHeader) {
        ...
        return (
          <p key={i} className="text-sm font-semibold text-gray-800 mt-3 mb-1 first:mt-0">
            {clean}
          </p>
        )
      }
  ...
  return (
    <div className={`flex items-end gap-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div
        className={`max-w-[75%] px-4 py-3 rounded-2xl shadow-sm space-y-1 ${
          isUser
...
export default function Chatbot() {
  ...
  // Hoist sendMessages above the effects that reference it
  ...
  // Initialise asynchronously so mode changes do not synchronously cascade renders.
  useEffect(() => {
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Powered by Groq · Context-aware</p>
            </div>
      ...
      <div className="bg-white border-t border-gray-100 px-6 py-4 shrink-0">
        <div className="max-w-3xl mx-auto flex gap-3">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition leading-relaxed"
          ...
          <button
            ...
            className="w-11 h-11 bg-blue-700 text-white rounded-xl flex items-center justify-center hover:bg-blue-800 disabled:opacity-40 disabled:cursor-not-allowed transition shrink-0 self-end"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
